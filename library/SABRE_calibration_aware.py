                ...
                if flag_write_syscode:
                    ...
                    # one qubit gate 타입에 따라 qasm instruction 의 포맷이 약간씩 다름
                        ...
                        if gate["gate"] in list_1q_rotations:
                            command = [gate["gate"]] + gate["angle"]
                            command.append(MT[gate["trgt"]])
                            list_syscode_commands.append(command)
                        ...
                        if angle is None:
                            # 알고리즘에 포함된 swap의 경우, 물리적 연산대신 labelling 만 다시 한다.
                            if gate["gate"] == swap:
                                list_syscode_commands.append(["Relabel", MT[gate["ctrl"]], MT[gate["trgt"]]])
                            
                            # non-parameteric 2-qubit gates: cnot, cz, 등
                            else:
                                list_syscode_commands.append([gate["gate"], MT[gate["ctrl"]], MT[gate["trgt"]]])
                        
                        # parameteric 2-qubit gates: cphase, rzz
                        else:
        ...
        #    5) 해당 SWAP 을 실제로 선택해서, 매핑 업데이트
            ...
            for node in FL:
                ...
                if node["gate"] in list_2q_gates:
                    ...
                    physical_ctrl = MT[ctrl_qubit]
                    physical_trgt = MT[trgt_qubit]
                    ...
                    physical_ctrl = MT[ctrl_qubit]
                    physical_trgt = trgt_qubit

                for neighbor in physical_connectivity[physical_ctrl]:
                    SWAP_candidate_list.append(((ctrl_qubit, trgt_qubit), ctrl_qubit, inverse_MT[neighbor]))
            ...
            if MT[best_SWAP[1]] not in physical_connectivity[MT[best_SWAP[2]]]:
...
def manage_synthesize(path_QASM, path_qchip, **kwargs):
    ...
    seed = synthesis_option.get("random_seed")
    if seed is not None:
        arguments.update({"seed": seed})
