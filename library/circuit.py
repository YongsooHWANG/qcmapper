...
class QuantumCircuit:
	...
	def one_qubit_gate(self, gate, qubit, **kwargs):
		...
		if gate in list_1q_rotations:
			list_angles = kwargs.get("angle")
			
			_str_code = [gate] + list_angles
			_str_code.append(qubit)

		elif gate in list_measure:
			cbit = kwargs.get("cbit") 
			if cbit is not None:
				self.system_code["cbit"].append(cbit)
				_str_code = [gate, qubit, cbit]
			else:
	...
	def manage_cnot(self, qubits, **kwargs):
		...
		elif final_operation == "Relabel":
			ic(qubits)
			self.system_code["circuit"].append([final_operation, qubits[0], qubits[1]])
			ctrl_qubit = self.inverse_mapping_table[qubits[0]]
			trgt_qubit = self.inverse_mapping_table[qubits[1]]

			qubit_mapping_table[ctrl_qubit],qubit_mapping_table[trgt_qubit] =\
				qubit_mapping_table[trgt_qubit],qubit_mapping_table[ctrl_qubit]
	...
	def get_analysis(self):
		...
		ret = {"Algorithm Qubits": len(self.system_code["qubit"])}
		if len(gate_depth_analysis):
			ret["Gate Depth"] = gate_depth_analysis
