...
def synthesize_dijkstra_manner(qasm_code, path_qchip, **kwargs):
    ...
    calibration = synthesis_option.get("calibration")

    calibration_type = synthesis_option.get("calibration_type")
    if calibration_type is None: calibration_type = "depth"
    
    if calibration and any(qchip_data.get(item) is None for item in ["cnot_gate_time", "cnot_error_rate"]):
    ...
    return True, best_performance_data
    ...
    for inst in qasm_code:
        ...
        # case 2: two-qubit gate: cnot 과 swap 고려함
            ...
            *angle, ctrl, trgt = inst[1:]
            ...
            # 알고리즘 상의 swap 은 물리적연산을 적용하지 않고, relabelling 함
            if gate == swap:
                qubit_mapping_table = _circuit.manage_cnot([ctrl_idx, trgt_idx], path=[],
                                                                                 qubit_table=qubit_mapping_table,
                                                                                 weight_graph=weight_graph,
                                                                                 calibration_type=calibration_type,
                                                                                 final_operation="Relabel")
