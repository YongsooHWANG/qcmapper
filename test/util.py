...
def display_qubit_movements(system_code, **kwargs):
	...
	qchip = kwargs.get("qchip")
	if qchip is None:
		raise Exception("Error : Quantum Chip is not provided")

	qchip_dimension = qchip.get("dimension")
	
	if qchip_dimension is None:
		lattice_size = {"width": 1, "height": len(qubit_mapping), "length": 1}
	else:
		lattice_size = qchip_dimension
	...
	print(" -----------------------------------------------------  ")
	pprint(qchip.get("qubit_connectivity"))
	print(" =====================================================  ")
	...
	for idx in range(circuit_depth):
		...
		flag_swap = False
		print(" =====================================================  ")
		print(" instructions at {}-th index :".format(idx))
		print(" -----------------------------------------------------  ")
		
		for inst in instructions:
			flag_swap = False	
			if inst[0] in ["swap", "cx", "cp", "rzz", "cz", "relabel"]:
				*angle, ctrl, trgt = inst[1:]
				ctrl, trgt = map(int, [ctrl, trgt])

				print(" {} qubits ({}, {}) -> ({}, {})".format(
					inst[0], ctrl, trgt, inverse_mapping.get(ctrl), inverse_mapping.get(trgt)))

				if inst[0] in ["swap", "relabel"]: 
					flag_swap = True
